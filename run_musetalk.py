...
6. Lo copia a output/final_lipsync.mp4.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Wrapper simple para MuseTalk 1.5 con barra de progreso")
    ...
    bbox_shift = args.bbox_shift if args.bbox_shift is not None else int(cfg.get("BBOX_SHIFT", "0"))
    use_float16 = (not args.no_float16) and as_bool(cfg.get("USE_FLOAT16", "true"), True)
    ...
    start_ts = time.time()
    log_file = ROOT / cfg.get("LOG_DIR", "logs") / "last_run.log"
    ...
    if latest is None:
        print("⚠️ MuseTalk terminó, pero no encontré ningún .mp4 en la carpeta de salida.")
